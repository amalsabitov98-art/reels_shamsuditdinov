...
const GEO_PATH = join(ROOT, 'scripts', '.flow-geo.json');
...
async function main() {
  if (!CHROME || !existsSync(CHROME)) { console.error('Chrome не найден. Запустите INSTALL.bat.'); process.exit(1); }
  ...
  if (!SKIP_GEO) {
    ...
    if (seen && seen !== 'US') {
      ...
      log('Страну автоматически определить не удалось. Доступность проверит сам Flow.');
    }
  }
