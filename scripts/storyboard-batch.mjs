...
for (const part of spec.parts) {
  ...
  // кадры под панели
  const partVideo = part.srcVideo
    ? resolve(part.srcVideo)
    : spec.srcDir
      ? resolve(spec.srcDir, `${spec.prefix || 'r'}${part.n}-video.mp4`)
      : resolve(spec.srcVideo);
  if (!existsSync(partVideo)) throw new Error(`нет видео части ${part.n}: ${partVideo}`);
  const framePaths = [];
